...
def run(injoblist):
    ...
    parser.add_option('-T', '--maxticket', default=100, help='maximum ticket value, if randomly assigned',
                      action='store', type='int', dest='maxticket')
    parser.add_option('-q', '--quantum', default=1, help='length of time slice', action='store', type='int',
                      dest='quantum')
    parser.add_option('-c', '--compute', help='compute answers for me', action='store_true', default=False,
                      dest='solve')
...
for i in range(1, 1000):
    command = str(i)+":"+str(maxlength)+","+str(i)+":"+str(maxlength)

    solution = run(command)
# Formel für die Unfairnes richtig Implementieren
# Formel = (finish_Time_of_FasterJob/finsh_Time_of_SlowerJob)
    print solution
    solution = (solution) / (float(i*2))
...
'''
#csv writer
with open('data.csv', mode='w') as doc:
    docwriter = csv.writer(doc)
    for entry in data:
        print entry
        docwriter.writerow(str(entry))
'''
